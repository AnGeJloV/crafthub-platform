...
import { Send, MessageSquare, Trash2, PackageOpen } from 'lucide-react';
...
export const ChatPage = () => {
    ...
    useEffect(() => {
        if (activeDialogueId) {
            ...
            if (diag) {
                ...
                void fetchProductDetails(diag.productId);
            } else {
                // Если чата нет, то подгрузим его позже
            }
    ...
    const handleDeleteDialogue = async (e: React.MouseEvent, dialogueId: number) => {
        e.stopPropagation();
        if (!window.confirm('Удалить диалог навсегда?')) return;
        try {
            ...
            <div className="w-1/3 bg-white rounded-3xl shadow-sm border border-slate-100 flex flex-col overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto">
                    {dialogues.length === 0 && !isDraftMode ? (
                        <div className="text-center p-10 text-slate-400 text-sm italic">История пуста</div>
                    ) : (
                        <>
                            {isDraftMode && (
                    ...
                    <>
                        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            <div className="flex items-center">
                                <div className="w-10 h-10 bg-indigo-600 text-white rounded-full flex items-center justify-center mr-3 font-black shadow-lg shrink-0">
                                    {(activeDialogue?.interlocutorName || draftName || "?")[0]}
                                </div>
                                <div>
                                    <h3 className="font-black text-slate-800 text-lg">{activeDialogue ? activeDialogue.interlocutorName : draftName}</h3>
                                    <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Собеседник</p>
                                </div>
                            </div>

                            {productDetails && (
                                <Link to={`/product/${productDetails.id}`} className="flex items-center bg-white p-2 rounded-2xl border border-slate-200 hover:border-indigo-300 transition-all shadow-sm max-w-[250px] group">
                                    <div className="h-10 w-10 rounded-xl overflow-hidden shrink-0 bg-slate-100">
                                        <img src={`http://localhost:8080/uploads/${mainImage}`} className="w-full h-full object-cover group-hover:scale-110 transition-all" alt="" />
                                    </div>
                                    <div className="ml-3 flex flex-col justify-center overflow-hidden pr-2">
                                        <span className="text-xs font-bold text-slate-800 truncate">{productDetails.name}</span>
                        ...
                        <form onSubmit={handleSendMessage} className="p-4 bg-white border-t border-slate-50 flex gap-3">
                            ...
                            <button type="submit" disabled={!newMessage.trim()} className="bg-indigo-600 text-white w-14 h-14 flex items-center justify-center rounded-2xl hover:bg-indigo-700 shadow-lg transition-all active:scale-90 disabled:bg-slate-200">
                                <Send size={20} />
                            </button>
                        </form>
                    </>
