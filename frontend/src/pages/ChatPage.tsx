...
export const ChatPage = () => {
    ...
    return (
        <div className="container mx-auto mt-4 px-4 h-[85vh] flex gap-6 pb-4">
                            ...
                            {dialogues.map(d => (
                                ...
                                >
                                    <div className="flex justify-between items-center mb-1">
                                        ...
                                        <button onClick={(e) => void handleDeleteDialogue(e, d.id)}
            ...
            {/* ПРАВАЯ ПАНЕЛЬ */}
            <div
                className="w-2/3 bg-white rounded-3xl shadow-sm border border-slate-100 flex flex-col overflow-hidden relative">
                    ...
                    <>
                        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            <div className="flex items-center">
                                ...
                                <Link to={`/profile/${activeDialogue?.interlocutorId || draftRecipientId}`}
                                      className="group flex items-center">
                                    <div
                                        className="w-10 h-10 bg-indigo-600 text-white rounded-full flex items-center justify-center mr-3 font-black shadow-lg shrink-0 group-hover:scale-105 transition-transform">
                            ...
                            {productDetails && (
                                <Link to={`/product/${productDetails.id}`}
                                      className="flex items-center bg-white p-2 rounded-2xl border border-slate-200 hover:border-indigo-300 transition-all shadow-sm max-w-[250px] group shrink-0 ml-4">
                                    <div className="h-10 w-10 rounded-xl overflow-hidden shrink-0 bg-slate-100">
                                        <img src={`http://localhost:8080/uploads/${mainImage}`}
                                    ...
                                    <div className="ml-3 flex flex-col justify-center overflow-hidden pr-2">
                                        <span
                                            className="text-xs font-bold text-slate-800 truncate leading-tight">{productDetails.name}</span>
                                        <span
                                            className="text-[10px] font-black text-indigo-600 mt-0.5">{productDetails.price} BYN</span>
                                    </div>
                                </Link>
                            )}
                        </div>
                        ...
                        <form onSubmit={handleSendMessage}
                            ...
                            <input type="text" placeholder="Ваше сообщение..."
                                   className="flex-1 bg-slate-100 border-none rounded-2xl px-6 py-4 text-sm focus:ring-2 focus:ring-indigo-500 transition-all font-medium outline-none"
                                   value={newMessage} onChange={(e) => setNewMessage(e.target.value)}/>
                            <button type="submit" disabled={!newMessage.trim()}
                                    className="bg-indigo-600 text-white w-14 h-14 flex items-center justify-center rounded-2xl hover:bg-indigo-700 shadow-lg transition-all active:scale-90 disabled:bg-slate-200 shrink-0">
                                <Send size={20}/>
                            </button>
                        </form>
                    </>
