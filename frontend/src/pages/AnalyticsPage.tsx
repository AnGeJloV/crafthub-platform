...
export const AnalyticsPage = () => {
    ...
    if (loading) return <div className="text-center mt-20 text-slate-400 animate-pulse font-black uppercase">Загрузка
        аналитики...</div>;
    ...
    return (
        <div className="container mx-auto mt-8 px-4 pb-20 max-w-6xl">
            <button onClick={() => navigate(-1)}
                    className="flex items-center text-slate-400 hover:text-indigo-600 mb-8 font-bold text-xs uppercase tracking-widest transition-colors group">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                <div
                    ...
                    <h3 className="text-3xl font-black text-slate-900">{stats.totalRevenue.toFixed(2)} <span
                        className="text-sm font-bold">BYN</span></h3>
                </div>
                ...
                <div
                    ...
                    <h3 className="text-3xl font-black text-slate-900">{stats.totalSales} <span
                        className="text-sm font-bold">сделок</span></h3>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                ...
                <div className="lg:col-span-3 bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100">
                    <h3 className="text-lg font-black text-slate-800 mb-8 uppercase tracking-widest">Динамика
                        выручки</h3>
                ...
                <div className="lg:col-span-2 bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100">
                    ...
                    <div className="space-y-6">
                        {stats.topProducts.length === 0 ? (
                            <p className="text-center text-slate-300 py-10 italic">Данные появятся после первых
                                продаж</p>
                        ) : stats.topProducts.map((item, idx) => (
                            <div key={idx} className="flex items-center justify-between">
                                <div className="flex items-center">
                                    <span
                                        className="w-6 h-6 bg-slate-100 text-slate-400 text-[10px] font-black rounded-lg flex items-center justify-center mr-4">{idx + 1}</span>
                                    <span
                                        className="text-sm font-bold text-slate-700 truncate max-w-[150px]">{item.name}</span>
                                </div>
                                <span
                                    className="bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full text-[10px] font-black uppercase">
                  {item.salesCount} шт.
                </span>
                            </div>
                        ))}
