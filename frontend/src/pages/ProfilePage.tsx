...
/**
 * Профиль пользователя: отображает данные мастера, его рейтинг и витрину товаров
 */
...
export const ProfilePage = () => {
    ...
    if (loading) return <div
        className="text-center mt-20 text-slate-400 font-bold animate-pulse uppercase tracking-widest text-xs">Загрузка профиля...</div>;
        ...
        <div className="container mx-auto px-4 py-8 pb-20 max-w-6xl">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div
                        className="bg-white rounded-4xl shadow-sm border border-slate-100 p-8 flex flex-col items-center relative">

                        <div className="relative mb-6">
                            <div
                                className="w-32 h-32 rounded-4xl overflow-hidden bg-slate-100 border-4 border-white shadow-xl">
                                <img
                            ...
                            {isMyProfile && (
                                <label
                                    className="absolute -bottom-2 -right-2 bg-slate-900 text-white p-2.5 rounded-2xl cursor-pointer hover:bg-indigo-600 transition-all border-4 border-white">
                                    <Camera size={18}/>
                        ...
                        {isEditing ? (
                            ...
                            <div className="text-center">
                                ...
                                <div className="flex items-center justify-center gap-2 mt-2">
                                    <span
                                        className="bg-indigo-50 text-indigo-600 text-[9px] font-black px-2 py-0.5 rounded-md uppercase tracking-widest">{profile.role.replace('ROLE_', '')}</span>
                                    <div
                                        className="flex items-center text-slate-400 text-[10px] font-bold uppercase tracking-tighter">
                        ...
                        <div className={`grid ${isSeller ? 'grid-cols-2' : 'grid-cols-1'} gap-4 w-full mt-8`}>
                            ...
                            {isSeller && (
                                <div className="bg-slate-50 p-4 rounded-3xl text-center border border-slate-100">
                                    <div className="text-yellow-500 mb-1 flex justify-center"><Star size={20} className="fill-yellow-500"/>
                                    </div>
                                    <div
                                        className="text-lg font-black text-slate-800">{(profile.averageRating || 0).toFixed(1)}</div>
                                    <div
                                        className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Рейтинг
                                    </div>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="w-full border-t border-slate-50 mt-8 pt-8 space-y-4">
                            <div className="flex items-center text-slate-500 text-sm font-medium"><Mail size={16} className="mr-4 text-indigo-400"/> {profile.email}
                            </div>
                            {!isEditing &&
                                <div className="flex items-center text-slate-500 text-sm font-medium"><Phone size={16} className="mr-4 text-indigo-400"/> {profile.phoneNumber}
                                </div>}
                        </div>
                    ...
                    {isSeller ? (
                        <>
                            <div className="flex items-center justify-between mb-8">
                                ...
                                <span
                                    className="bg-white px-3 py-1 rounded-full border border-slate-100 text-slate-400 font-bold text-xs">{profile.products.length} изделий</span>
                            </div>
                            {profile.products.length === 0 ? (
                                <div
                                    className="text-center py-24 bg-white rounded-4xl border-2 border-dashed border-slate-100">
                        ...
                        <div
                            className="h-full flex flex-col items-center justify-center bg-white rounded-4xl border border-slate-100 p-12 text-center shadow-sm min-h-[400px]">
                            <div
                                className="w-20 h-20 bg-indigo-50 text-indigo-600 rounded-3xl flex items-center justify-center mb-6 shadow-inner shadow-indigo-100/50">
                                <UserIcon size={40}/>
                            </div>
                ...
                <div
                    ...
                    <form onSubmit={handlePasswordChange} className="bg-white rounded-4xl p-10 max-w-md w-full shadow-2xl animate-in zoom-in duration-300">
                        ...
                        <div className="space-y-4 mb-8">
                            <input type="password" placeholder="Текущий пароль" required
                                   className="w-full border-2 border-slate-50 bg-slate-50 p-4 rounded-2xl outline-none focus:border-indigo-500 transition-all font-bold"
                        ...
                        <div className="flex gap-4">
                            <button type="submit" className="flex-1 bg-slate-900 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-lg active:scale-95">Обновить
                            </button>
                            <button type="button" onClick={() => setIsPassModalOpen(false)} className="flex-1 bg-slate-100 text-slate-400 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-200 transition-all">Отмена
                            </button>
                        </div>
                    </form>
                </div>
