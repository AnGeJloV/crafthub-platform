...
export const ProfilePage = () => {
    ...
    const handlePasswordChange = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            toast.error('Не удалось сменить пароль. Проверьте старый пароль.');
        }
    };
                        ...
                        {isEditing ? (
                            ...
                            <div className="text-center">
                                ...
                                <div className="flex items-center justify-center gap-2 mt-2">
                                    <span className={`text-[9px] font-black px-2 py-0.5 rounded-md uppercase tracking-widest bg-indigo-50 text-indigo-600`}>
                                        {profile.role === 'ROLE_ADMIN' ? 'Администратор' : profile.role === 'ROLE_SELLER' ? 'Продавец' : 'Покупатель'}
                                    </span>
                                ...
                                {isEditing ? (
                                    ...
                                    <>
                                        <button
                                            ...
                                            className="w-full flex items-center justify-center py-3 bg-slate-100 text-slate-500 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-200 transition-all active:scale-95"
                                        >
                                            <Edit3 size={16} className="mr-2" /> Редактировать
                                        </button>
                                        <button
                                            onClick={() => setIsPassModalOpen(true)}
                                            className="w-full flex items-center justify-center py-3 bg-slate-100 text-slate-500 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-all active:scale-95"
                                        >
                ...
                <div className="lg:col-span-2">
                    ...
                    {isAdmin ? (
                        <div className="h-full flex flex-col items-center justify-center bg-white rounded-4xl border border-slate-100 p-12 text-center shadow-sm min-h-[400px]">
                            <div className="w-24 h-24 bg-indigo-600 text-white rounded-[2rem] flex items-center justify-center mb-8 shadow-xl shadow-indigo-200">
                                <Shield size={48} />
                            </div>
                            ...
                            <p className="text-slate-500 max-w-md font-medium leading-relaxed mb-8">
                                {isMyProfile
                                    ? "Вы управляете всей платформой."
                                    : "Официальный представитель администрации CraftHub."}
                            </p>
                            {isMyProfile && (
                                <button
                                    onClick={() => navigate('/admin')}
                                    className="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-700 shadow-xl shadow-indigo-200 transition-all active:scale-95"
                                >
                ...
                <div className="fixed inset-0 z-100 flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4">
                    <form onSubmit={handlePasswordChange} className="bg-white rounded-4xl p-10 max-w-md w-full shadow-2xl animate-in zoom-in duration-300">
                        ...
                        <div className="flex gap-4">
                            ...
                            <button type="button" onClick={() => setIsPassModalOpen(false)} className="flex-1 bg-slate-100 text-slate-500 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-200 transition-all active:scale-95">Отмена</button>
                        </div>
                    </form>
                </div>
            )}
