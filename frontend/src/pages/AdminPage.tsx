...
import {UserCheck, PackageSearch, PlaySquare, Users, ShieldAlert, ShieldCheck, Mail, Phone} from 'lucide-react';
...
export const AdminPage = () => {
    const [activeTab, setActiveTab] = useState<'sellers' | 'products' | 'users'>('sellers');
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            const [sellers, products, allUsers] = await Promise.all([
    ...
    return (
        <div className="container mx-auto mt-8 px-4 pb-20">
            ...
            <div className="flex space-x-4 mb-10 bg-slate-100 p-1.5 rounded-2xl w-fit border border-slate-200">
                <button
                ...
                <div className="grid gap-6">
                    {sellerRequests.length === 0 ?
                        ...
                        sellerRequests.map(req => (
                            <div key={req.id}
                                 className="bg-white rounded-[2.5rem] border border-slate-100 p-8 flex flex-col md:flex-row gap-8 shadow-sm transition-all hover:shadow-md">
                                <div className="flex-1">
                                    ...
                                    <div className="flex space-x-3 mt-8">
                                        <button onClick={() => handleSellerDecision(req.id, 'approve')}
                                        ...
                                        <button onClick={() => handleSellerDecision(req.id, 'reject')}
                                ...
                                <div
                                    className="flex-1 bg-slate-50 rounded-[2rem] flex items-center justify-center cursor-zoom-in overflow-hidden h-72 border border-slate-100 group"
                                    onClick={() => {
                ...
                <div className="grid gap-6">
                    {productRequests.length === 0 ?
                        ...
                        productRequests.map(prod => (
                            <div key={prod.id}
                                 className="bg-white rounded-[2.5rem] border border-slate-100 p-8 flex flex-col lg:flex-row gap-10 shadow-sm transition-all hover:shadow-md">
                                <div
                                    className="lg:w-1/3 h-72 rounded-[2rem] overflow-hidden bg-slate-50 border border-slate-100">
                                    <img
                                            ...
                                            {prod.youtubeVideoId && (
                                                <a href={`https://youtube.com/watch?v=${prod.youtubeVideoId}`}
                                                   ...
                                                   className="flex items-center text-red-500 text-[10px] font-black uppercase tracking-widest hover:underline decoration-2 underline-offset-4">
                                                    <PlaySquare size={16} className="mr-1.5"/> Смотреть видео
                                                </a>
                                            )}
                                        </div>
                                    </div>
                                    <div className="flex space-x-4 mt-10">
                                        <button onClick={() => handleProductDecision(prod.id, 'approve')}
                                                className="flex-1 bg-green-600 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-green-700 transition-all shadow-lg shadow-green-100">ОДОБРИТЬ
                                        </button>
                                        <button onClick={() => handleProductDecision(prod.id, 'reject')}
                                    ...
                                    <tr key={u.id} className="hover:bg-slate-50/30 transition-colors">
                                        <td className="p-8">
                                            ...
                                            >
                                                ...
                                                <div>
                                                        <span
                                                                className="font-black text-slate-800 block text-sm leading-none mb-1 group-hover:text-indigo-600 transition-colors">
                                                                 {u.fullName}
                                                        </span>
                                                    {u.enabled ? (
                                                        <span
                                            ...
                                            <button
                                                ...
                                                className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-sm ${
                                                    isMe
                                                        ...
                                                        : u.enabled ? 'bg-red-50 text-red-500 hover:bg-red-500 hover:text-white' : 'bg-green-50 text-green-600 hover:bg-green-600 hover:text-white'
                                                }`}
                                            >
                ...
                >
                    ...
                    >
                        <SecureImage src={selectedImage}
                                     className="max-w-[90vw] max-h-[90vh] object-contain shadow-2xl rounded-3xl pointer-events-none"/>
                    </div>
                </div>
            )}
        </div>
